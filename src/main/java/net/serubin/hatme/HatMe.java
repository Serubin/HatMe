...
public class HatMe extends JavaPlugin {
	...
	public void onEnable() {
		...
		//Loads classes
		HatCommand Hat = new HatCommand(rbBlocks, rbAllow,
				notAllowedMsg, rbOp);
		AdminCommand Admin = new AdminCommand(rbBlocks, rbAllow,
				notAllowedMsg, rbOp);
	...
	public void remblock(Player player, String[] args) {
		// TODO Auto-generated method stub
		getConfig().getList("hatMe.allowed").remove(Integer.parseInt(args[1]));
		saveConfig();
		reloadConfig();
		rbBlocks = getConfig().getList("hatMe.allowed");
		player.sendMessage(ChatColor.GREEN + "Block " + args[1]
				+ " has been removed from the allowed block list!");
		log.warning("[" + name + "]: " + player + " has removed " + args[1] + "from the allowed blocks list!");

	}

	public void listblock(Player player, String[] args) {
		// TODO Auto-generated method stub
		// String allowedBlocks =
		// getConfig().getList("hatme.allowed").toString();
		rbBlocks = getConfig().getList("hatMe.allowed");
		String blocks = rbBlocks.toString();
		player.sendMessage("Allowed Blocks: " + blocks);
		// player.sendMessage(ChatColor.GREEN + "Allowed Blocks: " +
		// allowedBlocks);
	}

	public void addblock(Player player, String[] args) {
		// TODO Auto-generated method stub
		getConfig().getList("hatMe.allowed").add(Integer.parseInt(args[1]));
		saveConfig();
		reloadConfig();
		rbBlocks = getConfig().getList("hatMe.allowed");
		player.sendMessage(ChatColor.GREEN + "Block " + args[1]
				+ " has been added to the allowed block list!");
		log.warning("[" + name + "]: " + player + " has added " + args[1] + "to the allowed blocks list!");
	}
